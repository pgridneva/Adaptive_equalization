function BER = computeBERdfeNT(delay, dopplerShift, EbN0, param)
...
% dfe = comm.DecisionFeedbackEqualizer('Algorithm', 'RLS', ...
%     'NumFeedbackTaps', numTaps * SPS_eq, 'NumForwardTaps', numTaps * SPS_eq, ...
%     'ReferenceTap',refTap * SPS_eq, 'ForgettingFactor',param,...
%     'InputSamplesPerSymbol',SPS_eq);

% 
dfe = comm.DecisionFeedbackEqualizer('Algorithm', 'LMS', ...
    'NumFeedbackTaps', param * SPS_eq, 'NumForwardTaps', param * SPS_eq, ...
    'ReferenceTap',refTap * SPS_eq, 'StepSize',0.001,...
    'InputSamplesPerSymbol',SPS_eq);
...
% figure
    [signalOut,err] = dfe (signal, TrainingBaseband);
    reset(dfe)
%     plot(abs(err))
%         xlabel('Symbols')
